...
export default {
  ...
  methods: {
    ...
    regionDiscern(addressText) {
      ...
      const firstTwoKey = addressText.substring(0, 2) // 字符串开始的两个字符
      // 所在省的下标
      let provinceIndex = -1

      for (let i = 0; i < provinces.length; i++) {
        const { code, name } = provinces[i]
      ...
      for (let i = 0; i < citysArr.length; i++) {
        const { name, code } = citysArr[i]
        // 除去最后一个 市 字
        const cityName = name.substr(0, name.length - 1)
    ...
    addressDiscern(text) {
      ...
      const textArr = text.split(/[^\u4e00-\u9fa5a-zA-Z0-9+-（）()]+/g).filter(v => v.length)
